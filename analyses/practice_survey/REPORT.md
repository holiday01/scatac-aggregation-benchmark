...
**Question.** Is the aggregation step — the scale on which single cells are
combined into a per-cell-type profile — standardised in practice? This document
surveys 28 items (14 toolkit functions, 6 deconvolution/reference methods, 8
primary papers) and records, for each, the exact code line or Methods sentence
that establishes the scale.

Companion machine-readable file: `survey_table.csv`.
...
| aggregation class | n | coverage-invariant? |
|---|---:|---|
...
| pooled CPM (sum cells, then normalise) | 4 | yes |
| **unstated** (normalisation named but not specified, or absent) | **3** | **unknown** |
| **raw mean** (mean of counts, no per-cell normalisation) | **1** | **no — tracks coverage** |
...
coverage-reporting, or leave the scale unverifiable.** Restricting to the eight
primary research papers: 3 are coverage-safe only because they inherit an ArchR
...
### Two dot-plot functions, same operation, opposite scales
...
The two most used single-cell toolkits make this choice differently, and neither
warns the user.

### A trap inside a "safe" toolkit

ArchR is coverage-safe for gene scores only because `addGeneScoreMatrix` pre-normalises
...
| 3 | Seurat `AggregateExpression()` | 4.0.0+/5.x | NEWS 4.0.0: "Add `AggregateExpression()` for summation based pseudobulk calculations"; `method == "aggregate"` → sum then `NormalizeData` | pooled CPM | verified from code + docs |
...
| 10 | ArchR `getGroupSE()` / `plotEmbedding` / `plotBrowserTrack` | 1.0.x | `divideN = TRUE, scaleTo = NULL`; depth rescale is opt-in. `plotBrowserTrack` default `normMethod="ReadsInTSS"` | **raw mean by default** (tracks: pooled CPM) | verified from code |
...
| 17 | CIBERSORTx (single-cell signature) | 2019, Nat Biotech | "aggregated the profiles by summation in **non-log linear space** and normalized each population-level GEP into TPM" | pooled CPM | from Methods |
...
| 20 | scATAcat | 2024, NAR GAB | "adding the read coverage for each feature across the cluster member cells"; then "library size normalization and log2 transformation to the pseudobulk matrix" | pooled CPM | from Methods |
...
| 22 | Zhang et al., human chromatin accessibility atlas | 2021, Cell | "We aggregated open chromatin fragments from each cluster and utilized the promoter accessibility, defined as RPM of +/- 1kb around TSS, as the proxy for gene activity" | pooled CPM | from Methods |
...
1. **Luo et al. 2024, *Genome Biology*** (10.1186/s13059-024-03356-x),
   ...
   — and it quantifies library-size bias in embeddings ("LSI-based methods (Signac, ArchR)
...
3. **Teo et al. 2024, *Nat Commun*** (10.1038/s41467-024-53089-5), "Best practices
   ...
   gene-level activity scores and tests sequencing coverage as a performance factor,
...
4. **Benchmarking automated cell type annotation for scATAC-seq** (Genes 2022,
   ...
   coverage and library-size effects on scores and does not benchmark per-cell-type
   aggregation.
...
- No prior work benchmarks the aggregation scale of scATAC gene-activity matrices
  into per-cell-type profiles against sequencing coverage, or quantifies the
  resulting damage with an argmax-share read-out.
- The mean-of-logs problem itself is **not** new: it is well established in
  scRNA-seq (Lun 2018 onward) and enforced as an input rule by CIBERSORTx. Any
  claim to the contrary would be wrong.
- What the survey adds is that the choice is unstandardised in the scATAC
  literature: of 28 surveyed pipelines, 8 average on the log scale, 1 uses a raw
  mean by default, 1 publishes a z-scaled average of an unstated scale, and 3 do
  not state the scale at all.
- The companion benchmarks add that scATAC gene activity is sparse enough
  (1–18% detection per cell in the COAD matrix) for the effect to dominate rather
  than perturb, and that the log-normalised mean — the aggregation most
  practitioners treat as safe — is not safe at realistic coverage spreads.
...
- **Rows 23–25** (Braband, Suen, Farghli): the papers state the ArchR functions but
  ...
  is itself a finding — the coverage-safety of three of the eight primary papers is
